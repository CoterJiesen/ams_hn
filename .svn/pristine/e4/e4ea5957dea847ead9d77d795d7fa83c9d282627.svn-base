...
@IBDesignable open class LLCycleScrollView: UIView, UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate {
    ...
    open var views: Array<UIView> = [] {
        didSet {
           totalItemsCount = infiniteLoop! ? (views.count + imagePaths.count) * 100 : (views.count + imagePaths.count)
            if views.count != 1 {
            ...
            // 计算最大扩展区大小
            if scrollDirection == .horizontal {
                maxSwipeSize = CGFloat(views.count) * collectionView.frame.width
            }else{
                maxSwipeSize = CGFloat(views.count) * collectionView.frame.height
            }
    ...
    // MARK: 初始化
    open class func llCycleScrollViewWithFrame(_ frame: CGRect, views: Array<UIView>? = [], imageURLPaths: Array<String>? = [], titles:Array<String>? = [],didSelectItemAtIndex: LLdidSelectItemAtIndexClosure? = nil) -> LLCycleScrollView {
        let llcycleScrollView: LLCycleScrollView = LLCycleScrollView.init(frame: frame)
        
        if (views?.count)! > 0 {
            llcycleScrollView.views = views!
        }
    ...
    func setupPageControl() {
        ...
        if customPageControlStyle == .none {
            ...
            pageControl?.numberOfPages = self.imagePaths.count + self.views.count
        }
        ...
        if customPageControlStyle == .fill {
            ...
            (customPageControl as! LLFilledPageControl).pageCount = self.imagePaths.count + self.views.count
        ...
        if customPageControlStyle == .pill {
            ...
            (customPageControl as! LLPillPageControl).pageCount = self.imagePaths.count + self.views.count
            self.addSubview(customPageControl!)
        }
        ...
        if customPageControlStyle == .snake {
            ...
            (customPageControl as! LLSnakePageControl).pageCount = self.imagePaths.count + self.views.count
            self.addSubview(customPageControl!)
        }
    }
    ...
    override open func layoutSubviews() {
        ...
        if customPageControlStyle == .none || customPageControlStyle == .system {
            if pageControlPosition == .center {
                ...
                let pointSize = pageControl?.size(forNumberOfPages: self.imagePaths.count + self.views.count)
    ...
    func pageControlIndexWithCurrentCellIndex(index: NSInteger) -> (Int) {
        return Int(index % (imagePaths.count+views.count))
    }
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if isOnlyTitle && titles.count > 0{
            ...
            // 0==count 占位图
            if imagePaths.count == 0 && views.count == 0{
                cell.imageView.image = coverViewImage
            }else{
                let itemIndex = pageControlIndexWithCurrentCellIndex(index: indexPath.item)
                if imagePaths.count > 0  && views.count > 0  {
                    if views.count > itemIndex{
                        let iview = views[itemIndex]
                        iview.frame = cell.titleBackView.bounds
                        cell.titleBackView.addSubview(iview)
                    }else{
                        let imagePath = imagePaths[itemIndex-views.count]
                        // Mode
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if customPageControlStyle == .none || customPageControlStyle == .system {
            ...
            if scrollDirection == .horizontal {
                ...
                if currentOffsetX >= CGFloat(self.imagePaths.count + self.views.count) * scrollView.frame.size.width && infiniteLoop!{
                    collectionView.scrollToItem(at: IndexPath.init(item: Int(totalItemsCount/2), section: 0), at: position, animated: false)
                }
                ...
                if currentOffsetY >= CGFloat(self.imagePaths.count + self.views.count) * scrollView.frame.size.height && infiniteLoop!{
                    collectionView.scrollToItem(at: IndexPath.init(item: Int(totalItemsCount/2), section: 0), at: position, animated: false)
                }
