...
class RestitutionDeviceViewController:UIViewController,UITextFieldDelegate,scanDelegate,ZHDropDownMenuDelegate{
    ...
    override func viewDidLoad() {
        ...
        reportButton.addTarget(self, action: #selector(clickedRestitutionDevice), for: .touchUpInside)
    }
    override func viewWillAppear(_ animated: Bool) {
        getListRestitutionDevice()
    }

    func getListRestitutionDevice(){
        DataModel.restitutionDevice(username: CuUser.sharedInstance.username!){
    ...
    func clickedRestitutionDevice(){
        if selectedIndexs.count > 0 {
            ...
            DataModel.restitutionDeviceAction(strSns: strSns, username: CuUser.sharedInstance.username!){
                ...
                if response.success {
                    ...
                    if let value = response.value {
                        ...
                        vc.titleView.faultLabel.text = " 台返还失败"
                        vc.titleView.succLabel.text = " 台返还成功"
...
//        barCode.leftView!.addSubview(leftView)
        ...
        //按钮
        reportButton.setTitle("返还", for: UIControlState())
...
//重载tableview相关函数
extension RestitutionDeviceViewController:UITableViewDataSource,UITableViewDelegate{
