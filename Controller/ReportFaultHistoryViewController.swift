...
class ReportFaultHistoryViewController:UIViewController,UITextFieldDelegate{
    var reportHistoryList :[allocHistory]?
    ...
    override func viewWillAppear(_ animated: Bool) {
        getMangerReporFalutHistory()
    }
    
    func getMangerReporFalutHistory(){
        DataModel.reportFalutDeviceHistory(username: CuUser.sharedInstance.username!){
            (response:CuValueResponse<[allocHistory]>) -> Void in
            if response.success{
                if let value = response.value{
                    self.reportHistoryList =  value
...
//重载tableview相关函数
extension ReportFaultHistoryViewController:UITableViewDataSource,UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if let num = reportHistoryList?.count{
            return num
        }
        return 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let value = reportHistoryList?[indexPath.row]{
