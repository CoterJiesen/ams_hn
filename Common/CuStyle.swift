...
/// 自动维护APP的CSS文件 ,外界只需调用 CuStyle.sharedInstance.CSS 即可取得APP所需要的CSS
class CuStyle: NSObject {
    static let sharedInstance = CuStyle()
    ...
    dynamic var fontScale:Float {
        ...
        set{
            if _fontScale != newValue {
                _fontScale = newValue
                self.remakeCSS()
                CuSettings.sharedInstance[kFONTSCALE] = "\(_fontScale)"
            }
        }
    }
    var CSS = ""
    ...
    fileprivate override init() {
        super.init()
        //加载字体大小设置
        if let fontScaleString = CuSettings.sharedInstance[kFONTSCALE] , let scale = Float(fontScaleString){
    ...
    fileprivate func remakeCSS(){
        if let _ = CuColor.colors as? CuDefaultColor {
